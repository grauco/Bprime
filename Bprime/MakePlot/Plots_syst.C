...
//#define PATH "output/singleh/histos_Mo17_AN_22Feb/"
#define PATH "output/singleh/histos_ANv3/"
...
void Plots_syst(const char* channel, const char* syst, const char* histo){
  ...
  TFile *QCD_nom = new TFile((PATH+string("BprimeBToHB1000_")+channel+".root").c_str());
  //TFile *TT_nom = new TFile((PATH+string("TT_")+channel+".root").c_str());

  TFile *QCD_up = new TFile((PATH+string("BprimeBToHB1000_")+channel+"_"+syst+"Up.root").c_str());
  //TFile *TT_up = new TFile((PATH+string("TT_")+channel+"_"+syst+"Up.root").c_str());

  TFile *QCD_down = new TFile((PATH+string("BprimeBToHB1000_")+channel+"_"+syst+"Down.root").c_str());
 ...
 h_QCD_down->SetLineColor(kBlue);
  ...
  cat->AddText("at least 1 forward jet category");
  cat->SetBorderSize(0);
  cat->Draw();
  ...
  h_QCD_up->SetMaximum(0.6);
  h_QCD_up->SetMinimum(-0.6);
  ...
  canvas->Print((string("Plots_syst/")+syst+string("_")+histo+string("_")+channel+string("_B1000_acc.pdf")).c_str());
