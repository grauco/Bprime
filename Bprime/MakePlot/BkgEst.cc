...
void BkgEst(bool fromData){
  ...
  TString systs[15] = {"", "_puDown", "_puUp", "_mistagDown", "_mistagUp", "_btagDown", "_btagUp", "_q2Up", "_q2Down","_mistagHiggsUp", "_mistagHiggsDown", "_btagHiggsUp", "_btagHiggsDown", "_pdf_totalUp", "_pdf_totalDown"};
  TString cat[2] = {"cat1", "cat1"};
  for(int i=0; i<15; i++){
    for(int j=0; j<2; j++){
...
void doQCDTT(TString string, TString cat, bool fromData){
    ...
    TFile *qcd = new TFile("output/singleh/histos_nosplit/QCD_"+cat+"_singleH"+string+".root");
    TFile *tt = new TFile("output/singleh/histos_nosplit/TT_"+cat+"_singleH"+string+".root");
    TFile *data = new TFile("output/singleh/histos_nosplit/Data_"+cat+"_singleH"+string+".root");
    ...
    TFile *file = new TFile("output/singleh/histos_nosplit/QCDTT_"+cat+"_singleH"+string+".root", "UPDATE");
    ...
    cout << h_b->Integral() << " " << h_d->Integral() << endl;
    h_c->Scale(sf);
    cout << sf << endl;
    if(fromData==0){
    ...
    //h_c->GetXaxis()->SetRangeUser(200,2000);                                                                               
    //h_a->GetXaxis()->SetRangeUser(200,2000); 
    h_a->SetMaximum(5*h_a->GetMaximum());
    h_c->SetMaximum(5*h_a->GetMaximum());
    //h_c->SetMinimum(min(h_c->GetMinimum(), h_a->GetMinimum()));
    ...
    TString lumi_13TeV = "35.86 fb^{-1} (13 TeV)";
    ...
    TLine *line = new TLine(200,1.0,2000,1.0);
